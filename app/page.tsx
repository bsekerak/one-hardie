...
function ConciergeSection(props: ReturnType<typeof useHardieChat>) {
  ...
  return (
    <section className="flex-shrink-0" style={{ background: '#111111' }}>
      {/* Section label */}
      <div className="px-8 pt-8 pb-0 max-w-7xl mx-auto">
      ...
      {!started ? (
        /* ── Pre-start: prominent avatar + CTA ── */
        <div className="flex flex-col md:flex-row items-center gap-10 md:gap-16 px-8 py-14 max-w-5xl mx-auto">
          {/* Avatar */}
          ...
          <div className="flex-1 text-center md:text-left">
            <h2 className="text-white font-bold text-3xl lg:text-4xl leading-snug mb-4">
              Design your perfect<br />
              <span style={{ color: '#C49A3C' }}>whole-home exterior.</span>
            </h2>
            <p style={{ color: '#A89880' }} className="text-base leading-relaxed mb-8 max-w-lg">
            ...
            <button
              onClick={handleStart}
              className="inline-flex items-center gap-3 px-7 py-4 rounded-xl font-semibold text-base transition-all hover:scale-[1.02] active:scale-[0.98]"
            ...
            {ttsAvailable ? (
              <p style={{ color: '#6B5E52' }} className="text-xs mt-3 flex items-center gap-1.5">
                <Volume2 className="w-3.5 h-3.5" style={{ color: '#C49A3C' }} />
                Voice-enabled — Hardie will speak her responses aloud
              </p>
            ) : (
              <p style={{ color: '#6B5E52' }} className="text-xs mt-3">
        ...
        /* ── Post-start: chat UI ── */
        <div className="flex flex-col md:flex-row max-w-7xl mx-auto" style={{ borderTop: '1px solid #2C2A28' }}>
          {/* Avatar sidebar */}
          <div
            className="md:w-56 lg:w-64 flex-shrink-0 flex flex-col items-center justify-start px-6 py-8 gap-4"
            style={{ borderRight: '1px solid #2C2A28', borderBottom: 'none' }}
          >
          ...
          {/* Chat panel */}
          <div className="flex-1 flex flex-col min-h-[480px] max-h-[600px]" style={{ background: '#181818' }}>
            <ChatPanel
              messages={messages}
              input={input}
...
function PortfolioIntro() {
  return (
    <section className="flex-shrink-0 px-8 py-10 bg-white border-b border-gray-200">
...
function BrandCards() {
  return (
    <section className="flex-shrink-0 px-8 py-10 bg-gray-50 border-b border-gray-200">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-gray-900 font-bold text-2xl mb-1">Explore our industry-leading brands</h2>
        <p className="text-gray-500 text-sm mb-6">Together, our brands give homeowners and professionals more ways to create a beautiful, high-performing exterior.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {BRANDS.map((b) => (
            <div key={b.name} className="rounded-xl border bg-white p-5 hover:shadow-md transition-shadow" style={{ borderColor: b.border }}>
...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white overflow-y-auto">
      <TopNav />
      {/* Concierge is FIRST — above the hero */}
